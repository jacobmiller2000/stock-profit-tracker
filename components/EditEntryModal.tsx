...
interface EditEntryModalProps {
  entry: ProfitEntry
  onClose: () => void
  onSave: (entry: ProfitEntry) => void
}

export default function EditEntryModal({ entry, onClose, onSave }: EditEntryModalProps) {
  const [date, setDate] = useState(entry.date)
  const [realProfit, setRealProfit] = useState(entry.realAccountProfit.toString())
  const [paperProfit, setPaperProfit] = useState(entry.paperTradingProfit.toString())
  const [realPercentage, setRealPercentage] = useState(entry.realAccountPercentage.toString())
  const [paperPercentage, setPaperPercentage] = useState(entry.paperTradingPercentage.toString())
